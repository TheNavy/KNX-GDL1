...
if [ -e $ARDUINO_CLI_DIR ]; then
  echo "existing setup in $ARDUINO_CLI_DIR detected. Just updating library"
  updateLibrary;
  exit;
fi
...
#  * check for required command line tools like rsync and curl
#  * install Arduino CLI
#  * install boards
...
if [ -z $(which rsync) ] && [ -z $(which curl) ]; then
  echo "-> requires RSYNC and CURL! Stopping here."
  exit 1;
else
...
arduinocli lib install "extEEPROM"
arduinocli lib install "KonnektingFlashStorage"
arduinocli lib install "SerialFlash"
