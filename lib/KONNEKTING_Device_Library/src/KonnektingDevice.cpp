...
voidfct KonnektingDevice::_prepareSerialFunc = 0;
byte KonnektingDevice::_foo = 0;
// ---------------
...
void KonnektingDevice::internalInit(HardwareSerial &serial, word manufacturerID, byte deviceID, byte revisionID) {
    ...
    */
    ...
    if (deviceFlagDirty) {
        DEBUG_PRINTLN(F("WARNING: CRC issues found. Doing FactoryReset+Reboot now."));
        clearAllMemory();        
        memoryWrite(SYSTEMTABLE_DEVICE_FLAGS, 0xFF); 
        reboot(); 
    }
...
bool KonnektingDevice::waitForAck(byte ackCountBefore, unsigned long timeout) {
    ...
    while ((micros() < (start + timeout)) && _ackCounter == ackCountBefore) {
        if (micros() % 100 * 1000 /* every 100ms */ == 1) {
            DEBUG_PRINTLN(F("\tstart=%ld timeout=%ld micros=%ld before=%i ackCount=%i"), start, timeout, micros(), ackCountBefore, _ackCounter);
        }
        Knx.task();
    }
...
void KonnektingDevice::handleMsgAck(byte msg[]) {
    byte type = msg[2];
    DEBUG_PRINTLN(F("handleMsgAck type=0x%02x errCode=0x%02x"), type, /*errCode*/ msg[3]);
    if (type == 0x00) {
        _ackCounter++;
    }
}
...
bool KonnektingDevice::checkTableCRC(byte crcId){
    ...
    // memory index at which we find the current CRC32 value (4 bytes)
    int crcIndex = -1;
    switch(crcId) {
    ...
    /*
     * in factory settings, all memory cells have 0xFF
     * So, if CRC bytes have 4x 0xFF, it's very likely that 
     * memory has factory settings and found CRC value will 
     * never match calculated CRC value (based on also 0xFF cells)
     */
    if (crcValue==0xFFFFFFFF) {
        DEBUG_PRINTLN(F("crc checksum SUCCESS due to FACTORY 0xFFFFFFFF"));
        return true;
    }
...
void KonnektingDevice::handleMsgPropertyPageRead(byte msg[]) {
    ...
    if (addressFlag == 0x00 && isProgState()) {
        ...
        DEBUG_PRINTLN(F(" reply due to prog button"));
            ...
            (addressFlag = 0xFF) && 
            (_individualAddress == __WORD(msg[3], msg[4]))
        ) {
#ifdef DEBUG_PROTOCOL
...
void KonnektingDevice::clearAllMemory() {
    ...
    if (*_dataRemoveFunc != NULL) {            
        for (int typeId = 0x01; typeId<256; typeId++) {
            ...
            for (int dataId = 0; dataId<256; dataId+=1) {
                // [[maybe_unused]] --> https://en.cppreference.com/w/cpp/language/attributes
                [[maybe_unused]] bool res = _dataRemoveFunc(typeId, dataId);
    ...
    */
    // final commit after clearing memory
    memoryCommit();
}
...
void KonnektingDevice::handleMsgDataWriteFinish(byte msg[]) {
    ...
    if (*_dataCloseFunc != NULL) {
        ...
        if (thisCrc32 == otherCrc32) {
            ...
            [[maybe_unused]] bool result = _dataCloseFunc();
...
void KonnektingDevice::handleMsgDataRead(byte msg[]) {
    ...
    if (*_dataOpenReadFunc != NULL && *_dataReadFunc != NULL && *_dataCloseFunc != NULL) {
        ...
        if (size == 0) { // file with size 0 makes no sense, so must be > 0 in any non-error-case
...
void KonnektingDevice::setPrepareSerialFunc(voidfct func) {
    Konnekting._prepareSerialFunc = func;
}
